...
const OptimizationModal = ({ isOpen, onClose, originalListing, optimizedData, onApply }) => {
    ...
    const handleApply = () => {
        alert(t('optimization.success_message'));
        if (onApply) onApply();
        onClose();
    };
            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden border border-gray-100">
                ...
                <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-white z-10 shadow-sm relative">
                    <div>
                        <h2 className="text-2xl font-black text-gray-900 flex items-center">
                            ...
                            {t('optimization.comparison_title')}
                        </h2>
                        <p className="text-gray-500 text-sm mt-1 ml-14">{t('optimization.comparison_desc')}</p>
                    </div>
                ...
                <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 overflow-hidden relative">
                    ...
                    <div className="absolute left-1/2 top-[60%] transform -translate-x-1/2 -translate-y-1/2 z-30 hidden lg:flex flex-col items-center justify-center pointer-events-none">
                        <div className="bg-emerald-500 text-white font-black text-xl px-6 py-3 rounded-full shadow-2xl shadow-emerald-200 border-4 border-white flex items-center animate-bounce-slow">
                            ...
                            +{lqsDiff} {t('optimization.points_increase')}
                        </div>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-slate-100 via-indigo-50 to-blue-50 p-8 overflow-y-auto border-r border-gray-200 relative">
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <span className="bg-slate-200 text-slate-700 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider shadow-sm border border-slate-300">{t('optimization.original_listing')}</span>
                            <div className="flex items-center bg-white px-3 py-1 rounded-lg shadow-sm border border-indigo-100">
                                <span className="text-gray-500 font-bold mr-2 text-xs uppercase">{t('optimization.current_lqs')}</span>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-white p-5 rounded-2xl border-2 border-slate-200 shadow-sm hover:shadow-md transition-all group hover:border-blue-400">
                                <h3 className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-3 flex items-center group-hover:text-blue-600 transition-colors"><Type className="w-4 h-4 mr-1 text-slate-400 group-hover:text-blue-500" /> {t('optimization.title')}</h3>
                            ...
                            <div className="bg-white p-5 rounded-2xl border-2 border-slate-200 shadow-sm hover:shadow-md transition-all group hover:border-emerald-400">
                                <h3 className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-3 flex items-center group-hover:text-emerald-600 transition-colors"><DollarSign className="w-4 h-4 mr-1 text-slate-400 group-hover:text-emerald-500" /> {t('optimization.price')}</h3>
                            ...
                            <div className="bg-white p-5 rounded-2xl border-2 border-slate-200 shadow-sm hover:shadow-md transition-all group hover:border-purple-400">
                                <h3 className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-3 flex items-center group-hover:text-purple-600 transition-colors"><Tag className="w-4 h-4 mr-1 text-slate-400 group-hover:text-purple-500" /> {t('optimization.tags')} ({originalListing.tags?.length || 0})</h3>
                    ...
                    <div className="bg-white p-8 overflow-y-auto relative">
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <span className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider shadow-lg shadow-indigo-200">{t('optimization.ai_suggestion')}</span>
                            <div className="flex items-center">
                                <span className="text-indigo-900 font-bold mr-2">{t('optimization.target_lqs')}</span>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="bg-indigo-50/50 p-5 rounded-2xl border border-indigo-100 shadow-sm ring-1 ring-indigo-50">
                                <h3 className="text-indigo-400 text-xs font-bold uppercase tracking-wider mb-3 flex items-center"><Type className="w-4 h-4 mr-1" /> {t('optimization.suggested_title')}</h3>
                                <p className="text-indigo-900 font-bold leading-relaxed text-lg">{optimizedData.suggested_title || t('optimization.no_suggestion')}</p>
                            </div>

                            {/* Price */}
                            <div className="bg-emerald-50/50 p-5 rounded-2xl border border-emerald-100 shadow-sm ring-1 ring-emerald-50">
                                <h3 className="text-emerald-600 text-xs font-bold uppercase tracking-wider mb-3 flex items-center"><DollarSign className="w-4 h-4 mr-1" /> {t('optimization.optimal_price')}</h3>
                                <div className="flex items-end">
                                    ...
                                    <span className="ml-3 text-sm text-emerald-600 font-medium mb-1.5 bg-emerald-100 px-2 py-0.5 rounded">
                                        {optimizedData.predicted_price_optimal > originalListing.price ? t('optimization.increase_suggested') : t('optimization.decrease_suggested')}
                                    </span>
                                </div>
                            ...
                            <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-lg shadow-gray-100">
                                <h3 className="text-indigo-400 text-xs font-bold uppercase tracking-wider mb-3 flex items-center"><Tag className="w-4 h-4 mr-1" /> {t('optimization.suggested_tags')}</h3>
                                <div className="flex flex-wrap gap-2">
                                ...
                                <p className="text-xs text-gray-400 mt-3 italic">{t('optimization.new_tags_note')}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 border-t border-gray-100 bg-white flex justify-between items-center z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                    <div className="text-sm text-gray-500 font-medium">
                        <span className="font-bold text-gray-900">{t('optimization.note_label')}</span> {t('optimization.note_text')}
                    </div>
                    <div className="flex space-x-4">
                        <button onClick={onClose} className="px-6 py-3 rounded-xl font-bold text-gray-500 hover:bg-gray-100 transition-colors">{t('common.cancel')}</button>
                        <button onClick={handleApply} className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-xl font-bold shadow-xl shadow-indigo-200 transition-all transform hover:-translate-y-1 flex items-center text-lg">
                            <CheckCircle className="w-5 h-5 mr-2" /> {t('optimization.apply_changes')}
                        </button>
                    </div>
                </div>
            </div>
