    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12 mb-16 animate-fade-in">
            <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-lg hover:shadow-xl transition-shadow">
                <h3 className="text-xl font-bold text-gray-800 mb-6 flex items-center"><PieIcon className="w-6 h-6 mr-3 text-indigo-500" /> LQS Performans Dağılımı</h3>
            ...
            <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-lg hover:shadow-xl transition-shadow">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold text-gray-800 flex items-center"><BarChart2 className="w-6 h-6 mr-3 text-indigo-500" /> {rightChartType === 'price' ? 'Fiyat Analizi' : 'Materyal Dağılımı'}</h3>
                    <div className="flex bg-gray-100 p-1.5 rounded-xl">
                        <button onClick={() => setRightChartType('price')} className={`px-4 py-2 text-xs font-bold rounded-lg transition-all ${rightChartType === 'price' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>Fiyat</button>
                        <button onClick={() => setRightChartType('materials')} className={`px-4 py-2 text-xs font-bold rounded-lg transition-all ${rightChartType === 'materials' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>Materyal</button>
                    </div>
                </div>
                <div className="h-72 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={rightChartData} layout={rightChartType === 'materials' ? 'vertical' : 'horizontal'}>
                            ...
                            <Bar dataKey="value" fill="#6366f1" radius={[6, 6, 6, 6]} barSize={32} name={rightChartType === 'price' ? 'Fiyat ($)' : 'Adet'} />
                        </BarChart>
                    </ResponsiveContainer>
                </div>
            </div>
